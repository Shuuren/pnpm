...
#[test]
fn matched_scripts_that_depend_on_each_other_run_in_graph_order() {
    let CommandTempCwd { pacquet, root, workspace, .. } = CommandTempCwd::init();
    write_workspace(
        &workspace,
        &[(
            ...
            json!({
                ...
                "scripts": {
                    ...
                    "other": append_line_script("other", "../order.log"),
    ...
    pacquet
        .with_args(["-r", "run", "/^(build|test|other)$/"])
        .assert()
        .success();
    ...
    let test_at = lines
        .iter()
        .position(|line| *line == "test")
        .expect("test");
    assert!(build_at < test_at);
    lines.sort_unstable();
    assert_eq!(lines, ["build", "other", "test"]);
