...
test('matched scripts that depend on each other run in graph order', async () => {
  await using server = await createTestIpcServer()
  ...
  preparePackages([
    {
      ...
      scripts: {
        ...
        other: server.sendLineScript('other'),
  ...
  const order = server.getLines()
  expect([...order].sort()).toStrictEqual(['build', 'other', 'test'])
